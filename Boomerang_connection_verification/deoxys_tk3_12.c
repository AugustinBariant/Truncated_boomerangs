...
int main () {
  ...
  state delta_K[4] = {
    ...
    _mm_setr_epi32(0x007b0000,0x7b000000,0x0000008d,0x0000f600),
    _mm_setr_epi32(0x00000000,0x00000000,0x00000000,0x50b1b1e1),
    _mm_setr_epi32(0xb2000000,0x00080000,0x0000ba00,0x000000ba),
  ...
  state nabla_K[4] = {
    //               03020100   07060504   0b0a0908   0f0e0d0c
    _mm_setr_epi32(0x00000000,0x00000000,0x00000000,0x00000000),
    _mm_setr_epi32(0x00000000,0x0000a200,0x00000000,0x0071004a),
    _mm_setr_epi32(0x2e000000,0x00000041,0x00006000,0x00000000),
    _mm_setr_epi32(0x00000000,0x00000000,0xed720040,0x00000000),
  };
...
#pragma omp for
    for (uint64_t sample=0; sample<SAMPLES; sample+=CHUNK) {
      ...
      while (a == -1) {
          ...
          int y = x ^ 0xff;
          if ((SBox[x] ^ SBox[y]) == 0x75)
            a = x;
      }
      while (b == -1) {
          int x = aesrand_int32_r(rnd_state) & 0xff;
          int y = x ^ 0x5e;
          if ((SBox[x] ^ SBox[y]) == 0xe2)
            b = x;
      }
      while (c == -1) {
          int x = aesrand_int32_r(rnd_state) & 0xff;
          int y = x ^ 0x0b;
          if ((SBox[x] ^ SBox[y]) == 0x48)
            c = x;
      }
      ...
      for (uint64_t t=0; t<CHUNK; t++) {
        ...
        state mask = _mm_setr_epi32(0xff00ffff,0x00ffffff,0xffffff00,0xffffffff);
        X = (X & mask) | _mm_setr_epi32(b<<16, c<<24, a, 0);
        ...
        if (1) {
          ...
          if (!_mm_eq(U ^ _mm_setr_epi32(0x5e0000, 0x0b000000, 0xff, 0), V)) {
            print_state(U^V);
            print_state(_mm_setr_epi32(0x5e0000, 0x0b000000, 0xff, 0));
          }
          assert(_mm_eq(U ^ _mm_setr_epi32(0x5e0000, 0x0b000000, 0xff, 0), V));
        }
