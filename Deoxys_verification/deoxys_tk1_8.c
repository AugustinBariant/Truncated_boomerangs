...
int main () {
  ...
  state delta_K[4] = {
    ...
    _mm_setr_epi32(0x3b000000,0x00000000,0x00007b00,0x00000000),
    _mm_setr_epi32(0x00000000,0x00000000,0x003b007b,0x00000000),
    _mm_setr_epi32(0x00000000,0x00003b00,0x00000000,0x7b000000),
  ...
  state nabla_K[4] = {
    //               03020100   07060504   0b0a0908   0f0e0d0c
    _mm_setr_epi32(0x00000000,0x00000000,0x00000000,0x00000000),
    _mm_setr_epi32(0x00000000,0x00000000,0x00000000,0x00c2006a),
    _mm_setr_epi32(0x6a000000,0x00000000,0x0000c200,0x00000000),
    _mm_setr_epi32(0x00000000,0x00000000,0x006a00c2,0x00000000),
  };
...
#pragma omp for
    for (uint64_t sample=0; sample<SAMPLES; sample+=CHUNK) {
      ...
      while (a == -1) {
          ...
          int y = x ^ 0x9b;
          if ((SBox[x] ^ SBox[y]) == 0xb7)
            a = x;
      }
      while (b == -1) {
          int x = aesrand_int32_r(rnd_state) & 0xff;
          int y = x ^ 0x13;
          if ((SBox[x] ^ SBox[y]) == 0x54)
            b = x;
      }
      ...
      for (uint64_t t=0; t<CHUNK; t++) {
        ...
        state mask = _mm_setr_epi32(0xff00ffff,0xffffffff,0xffffff00,0xffffffff);
        X = (X & mask) | _mm_setr_epi32(a<<16, 0, b, 0);
...
#pragma omp critical
  ...
  printf ("Good pairs: %llu\np = 2^%f\nSAMPLES = %llu", (long long) count, log2(1.0*count/SAMPLES), (long long) SAMPLES);
}
