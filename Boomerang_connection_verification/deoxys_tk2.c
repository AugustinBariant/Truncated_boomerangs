...
int main () {
  ...
  state delta_K[4] = {
    ...
    _mm_setr_epi32(0x00000000,0x00000000,0x0000ee00,0x00000000),
    _mm_setr_epi32(0x00000000,0x00000000,0x00000032,0x00000000),
    _mm_setr_epi32(0x00000000,0x00000000,0x00000000,0x8b000000),
  ...
  state nabla_K[4] = {
    //               03020100   07060504   0b0a0908   0f0e0d0c
    _mm_setr_epi32(0x00000000,0x00000000,0x00000000,0x00000000),
    _mm_setr_epi32(0x00000000,0x00a90000,0x00000000,0x00005a00),
    _mm_setr_epi32(0x00002700,0x00000000,0x00000000,0x00000029),
    _mm_setr_epi32(0xce00003a,0x00000000,0x00000000,0x00000000),
  };
...
#pragma omp for
    for (uint64_t sample=0; sample<SAMPLES; sample+=CHUNK) {
      ...
      while (a == -1) {
          ...
          int y = x ^ 0x3a;
          if ((SBox[x] ^ SBox[y]) == 0xb3)
            a = x;
      }
      while (b == -1) {
          int x = aesrand_int32_r(rnd_state) & 0xff;
          int y = x ^ 0xcb;
          if ((SBox[x] ^ SBox[y]) == 0x7a)
            b = x;
      }
      while (c == -1) {
          int x = aesrand_int32_r(rnd_state) & 0xff;
          int y = x ^ 0x69;
          if ((SBox[x] ^ SBox[y]) == 0x3d)
            c = x;
      }
      ...
      for (uint64_t t=0; t<CHUNK; t++) {
        ...
        state mask = _mm_setr_epi32(0xffffff00,0xffffffff,0xff00ffff,0x00ffffff);
        X = (X & mask) | _mm_setr_epi32(a, 0, b<<16, c<<24);
        ...
        if (0) {
          ...
          if (!_mm_eq(U ^ _mm_setr_epi32(0x3a, 0, 0xcb0000, 0x69000000), V)) {
            print_state(U^V);
          }
          assert(_mm_eq(U ^ V, _mm_setr_epi32(0x3a, 0, 0xcb0000, 0x69000000)));
        }
...
#pragma omp critical
  ...
  printf ("Good pairs: %llu\np = 2^%f", (long long) count, log2(1.0*count/SAMPLES));
}
